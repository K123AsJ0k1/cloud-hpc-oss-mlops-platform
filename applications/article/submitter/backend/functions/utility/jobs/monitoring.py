                ...
                for job_key, job_object in job_objects.items():
                    ...
                    if job_object_data['submit'] and not job_object_data['stopped']:
                        ...
                        if not job_id in current_jobs:
                            # Squeue only shows active jobs
                            # Thus, jobs that are not seen are 
                            # either complete or failed
                            # This can be seen in sacct,
                            # which means the only relevant 
                            # states are pending, running and stopped
