...
def get_folders_and_files(
    ...
    # Change later to handle projappl and scratch
    resulted_prints = run_remote_commands(
        platform_secrets = platform_secrets,
        commands = [
            'ls'
...
def get_venvs(
    ...
    for folder in current_folders:
        if 'venv' in folder:
            # Refactor later to handle multiple modules
...
def get_modules(
    ...
    print('modules')
    # Gives a empty string for some reason
    resulted_prints = run_remote_commands(
        target_secrets = target_secrets,
        commands = [
            'source /appl/profile/zz-csc-env.sh && module list'
        ]
    )
    print(resulted_prints)
    modules = None
    if 0 < len(resulted_prints):
        modules = format_supercomputer_modules(
            resulted_print = resulted_prints[0]
        )
    return modules
'''