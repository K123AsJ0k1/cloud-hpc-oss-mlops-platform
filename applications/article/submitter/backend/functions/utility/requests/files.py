...
def send_file(
    ...
    # We assume that sources are always
    # platform/storage/path format 
    # with | specifying dict keys
    # Thus, submitter/secrets|integratio|cpouta-mahti|key uses the path
    # and allas/pipeline/code/slurm/ray-cluster.sh
    # here the name is used as the name in the target
    ...
    if source_split[0] == 'local':
        if source_split[1] == 'submitter':
            if 'secrets' in source_split[2]:
                ...
                # We assume that all credentials are in personal directory
