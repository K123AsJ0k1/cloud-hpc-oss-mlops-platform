...
from functions.management.artifacts import fetch_job_status, fetch_bridge_status, fetch_job_seff, fetch_job_sacct, fetch_job_logs
...
# Created and works
@artifacts.route("/job/bridges", methods=["GET"])
def get_bridges():
    sent_payload = json.loads(request.json)
    kubeflow_user = sent_payload['kubeflow-user']
    payload = fetch_bridge_status(
        file_lock = current_app.file_lock,
        allas_client = current_app.allas_client,
        allas_bucket = current_app.allas_bucket,
        kubeflow_user = kubeflow_user
